...
import apiService from '../ApiController/ApiService'; // Adjust the path based on your project structure
...
export const FAQSection = () => {
    ...
    useEffect(() => {
        const fetchFaqs = async () => {
            try {
                setLoading(true);
                const response = await apiService.getAllFaq();
                // Transform API response to match component's expected structure
                const transformedFaqs = response.map(faq => ({
                    question: faq.question,
                    answer: faq.answer
                }));
    ...
    if (loading) {
        return (
            <div className="mx-auto bg-[#131060] p-5 m-5 rounded-[11px]">
                <h2 className="text-xl font-semibold text-white mb-5 mt-3">Support FAQ</h2>
