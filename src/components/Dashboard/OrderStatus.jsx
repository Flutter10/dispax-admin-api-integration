...
const OrderStatus = () => {
  ...
  // Update apiService to accept pagination parameters
  const fetchShipmentsData = async (tab, page, companyId) => {
    setLoading(true);
    setError(null);
    try {
      ...
      const response = await (tab === "Pending"
        ? apiService.getAllPendingShipment(params)
        : apiService.getAllCompletedShipment(params));

      // Since apiService returns only docs, we need to adjust the response handling
      // Assuming apiService is updated to return the full response
      const shipmentDocs = response.data?.docs || response;
      setShipments(shipmentDocs);
  ...
  // Fetch shipments when tab, page, or company changes
  useEffect(() => {
    fetchShipmentsData(selectedTab, currentPage, selectedCompany);
  }, [selectedTab, currentPage, selectedCompany]);
    ...
    <div className="text-white w-full p-6">
      ...
      <div className="mb-6">
        {/* <select
          ...
          className="w-[220px] h-[48px] bg-[#070539] text-white rounded-[5px] p-2"
        >
