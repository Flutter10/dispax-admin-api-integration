...
const Revenue = () => {
  ...
  useEffect(() => {
    const fetchRevenue = async () => {
      ...
      setError(null);
      try {
        const response = await apiService.getRevenue();
        ...
        setTotalRevenue(response.data?.summary?.formattedTotalRevenue || "$0.00");
      } catch (err) {
    ...
    <div className="text-white w-full p-6">
      ...
      <div className="grid grid-cols-1 mb-6">
        <div className="bg-[#131060] rounded-lg p-6">
          <h3 className="text-[12px] text-gray-300 mb-2">Total Revenue</h3>
          <div className="flex gap-2">
            <span className="text-[22px] font-bold mb-1">{totalRevenue}</span>
          </div>
        </div>
      </div>

      {/* Revenue Chart */}
      <div className="bg-[#131060] border border-[#1C1889] rounded-lg p-6">
        <div className="flex justify-between items-center mb-4">
          <div>
        ...
        {loading ? (
          ...
          <p className="text-white text-center py-4">No revenue data available.</p>
        ) : (
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={revenueData}>
