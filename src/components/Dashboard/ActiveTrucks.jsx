...
const ActiveTrucksData = () => {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        ...
        const companyData = Array.isArray(response?.docs) ? response.docs : [];
        console.log('Fetched companies:', companyData);
        setCompanies(companyData);
        if (companyData.length > 0) {
          setSelectedCompany(companyData[0]._id);
        }
      } catch (err) {
  ...
  useEffect(() => {
    const fetchTrucks = async () => {
      if (!selectedCompany) {
        console.log('No company selected, skipping truck fetch');
        return;
      }
      try {
        setLoading(true);
        setError(null);
        const response = await apiService.getAllTruck(currentPage, 10);
        console.log('Fetched trucks:', response);
        const trucks = response.docs.map((truck) => ({
          id: truck._id,
          TruckId: truck.truckNumber,
          Model: truck.modelYear,
          Capacity: "Unknown",
          TotalTrips: truck.company.totalTrip || 0,
          Status: truck.truckStatus,
          Location: "Unknown",
          LastMaintenance: truck.updatedAt.split("T")[0],
        }));
        setTrucksData(trucks);
        setTruckTotalPages(response.totalPages);
      } catch (err) {
        setError("Failed to fetch trucks data");
  ...
  }, [currentPage, selectedCompany, selectedTab]);

  // Fetch drivers data
  useEffect(() => {
    const fetchDrivers = async () => {
      if (!selectedCompany) {
        console.log('No company selected, skipping driver fetch');
        return;
      }
      try {
        setLoading(true);
        setError(null);
        const response = await apiService.getAllDriver(currentPage, 10);
        console.log('Fetched drivers:', response);
        const allDrivers = response.docs;
        const filteredDrivers = allDrivers.filter((driver) => driver.teamAdminId === selectedCompany);
        console.log('Filtered drivers:', filteredDrivers);
        const drivers = filteredDrivers.map((driver) => ({
          id: driver._id,
          DriverId: driver.Id || "N/A",
          Name: driver.fullName,
          Contact: driver.mobileNumber || "N/A",
          AssignedTrucks: "Unknown",
          Status: driver.userStatus,
          ActiveTrips: driver.totalShipment > 0 ? "In Transit" : "None",
          WorkingHours: `${driver.workingHours} Hours`,
        }));
        setDriversData(drivers);
        // Calculate total pages based on filtered drivers
        const totalFilteredDrivers = allDrivers.reduce((count, driver) => {
          return driver.teamAdminId === selectedCompany ? count + 1 : count;
        }, 0);
        const calculatedTotalPages = Math.ceil(totalFilteredDrivers / 10);
        setDriverTotalPages(calculatedTotalPages || 1); // Ensure at least 1 page
        console.log('Total filtered drivers:', totalFilteredDrivers, 'Calculated total pages:', calculatedTotalPages);
        // Reset currentPage if it exceeds calculatedTotalPages
  ...
  const handlePageChange = (page) => {
    const totalPages = selectedTab === "Trucks" ? truckTotalPages : driverTotalPages;
    ...
    return (
      <div className="flex items-center gap-2">
        <button
          ...
          className={`px-3 py-1 rounded ${currentPage === 1 ? 'bg-gray-500 cursor-not-allowed' : 'bg-[#0A0666] text-white'}`}
        >
          Previous
        </button>
        {pages.map((page) => (
          <button
            ...
            className={`px-3 py-1 rounded ${currentPage === page ? 'bg-[#0A0666] text-white' : 'bg-[#131060] text-white'}`}
          >
            {page}
          </button>
        ))}
        <button
          ...
          className={`px-3 py-1 rounded ${currentPage === totalPages ? 'bg-gray-500 cursor-not-allowed' : 'bg-[#0A0666] text-white'}`}
        >
          Next
        </button>
      </div>
  ...
  const renderTable = () => {
    ...
    if (selectedTab === "Trucks" && trucksData.length === 0) {
      return <div className="text-white">No trucks available</div>;
    }
    if (selectedTab === "Drivers" && driversData.length === 0) {
      return <div className="text-white">No drivers available</div>;
    }
    ...
    <div className="text-white w-full p-6">
      ...
      <div className="flex justify-between items-center mt-4">
        <p className="text-sm text-white/70">
          Showing {(selectedTab === "Trucks" ? trucksData : driversData).length} out of {(selectedTab === "Trucks" ? truckTotalPages : driverTotalPages) * 10} results
        </p>
        {renderPagination()}
      </div>
    </div>
