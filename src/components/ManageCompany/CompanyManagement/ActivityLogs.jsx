...
const ActivityLogs = () => {
  ...
  const [companyName, setCompanyName] = useState('TransGlobal Logistics');

  // Transform API response to match UI structure
  const transformLogs = (apiLogs) => {
    return apiLogs.map((log) => {
      const isTruckRelated = log.title.toLowerCase().includes('truck') || log.description.toLowerCase().includes('truck');
      return {
        type: log.title.toLowerCase().includes('user updated') ? 'admin-login' : isTruckRelated ? 'truck-assignment' : 'new-driver',
        ...
        user: log.user || 'Unknown User', // User ID not resolved to name; using placeholder
        avatar: log.user ? log.user.slice(0, 2).toUpperCase() : 'UU', // Derive avatar from user ID
        tags: isTruckRelated ? ['Truck ID - Unknown', 'Driver ID - Unknown'] : [], // Placeholder tags
        hasTruckIcon: isTruckRelated,
      };
    });
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const company = await apiService.getCompanyById(id);
        setCompanyName(company.fullName || 'TransGlobal Logistics');

        // Fetch activity logs
        const logData = await apiService.getCompanyLogActivity(id);
        const transformedLogs = transformLogs(Array.isArray(logData) ? logData : [logData]);
        ...
        await Swal.fire({
          icon: 'error',
          title: 'Error',
          text: error.message || 'Failed to load activity logs. Please try again.',
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        <p className="text-[22px] font-semibold mb-1">Activity Logs</p>
        <p className="text-[16px] text-[#B2B2B2] font-medium mb-6">
          {companyName} - Recent activities and system logs
        </p>
      </div>
                  ...
                  <div className="">
                    <div className="flex mb-2">
                      {log.hasTruckIcon ? (
                        ...
                        <img src="/profileIcon.svg" alt="Profile" className="w-5 h-5 mr-2" />
                      )}
                      <p className="text-white text-[16px] font-medium">{log.message}</p>
                    </div>
                    <p className="text-[#B2B2B2] text-[14px] font-medium">
