...
const DispatchingCompanyTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [packageData, companyResponse] = await Promise.all([
          apiService.getPackage(),
          apiService.getCompany({ page: pagination.page, limit: pagination.limit }),
        ]);
        setPackages(packageData);
        ...
        setPagination({
          ...
          limit: companyResponse.limit,
          totalDocs: companyResponse.totalDocs,
          totalPages: companyResponse.totalPages,
          hasPrevPage: companyResponse.hasPrevPage,
          hasNextPage: companyResponse.hasNextPage,
        });
      } catch (error) {
        console.error('Error fetching data:', error);
      }
  ...
  const handleCreateCompany = async () => {
    ...
    try {
      ...
      const companyResponse = await apiService.getCompany({ page: pagination.page, limit: pagination.limit });
      const packageMap = packages.reduce((map, pkg) => {
        map[pkg.id] = pkg.title;
        return map;
      }, {});
    ...
    <>
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex items-center gap-2">
          <button
            className={`px-4 py-2 text-white rounded-lg ${pagination.hasPrevPage ? 'bg-[#6F1AFF]' : 'bg-[#858080] cursor-not-allowed'}`}
          ...
          <button
            className={`px-4 py-2 text-white rounded-lg ${pagination.hasNextPage ? 'bg-[#6F1AFF]' : 'bg-[#858080] cursor-not-allowed'}`}
