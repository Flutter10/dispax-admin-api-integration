...
import apiService from '../ApiController/ApiService'; // Assuming apiService is in the same directory
...
const SubscriptionPlan = () => {
  ...
  const openEditPopup = async (role) => {
    try {
      const packageData = await apiService.getPackageById(role.id);
      setEditPopup({ open: true, role });
      setPackageId(role.id);
      setRoleName(packageData.name.charAt(0).toUpperCase() + packageData.name.slice(1));
      setDescription(packageData.description || '');
      setPrice(packageData.isFree ? '$0' : `$${packageData.price}`);
      setShowDropdown(null);
    } catch (error) {
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Failed to fetch package details. Please try again.',
         confirmButtonColor: '#6F1AFF',
        customClass: {
          confirmButton: 'swal-confirm-button',
        },
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const packageData = {
        ...
        description,
        price: priceValue === '0' ? 0 : parseFloat(priceValue),
        isFree: priceValue === '0',
      };
      await apiService.updatePackage(packageId, packageData);
      Swal.fire({
        icon: 'success',
        title: 'Success',
      ...
      // Refresh packages
      const updatedPackages = await apiService.getPackage();
  ...
  const handleDelete = async (packageId, roleName) => {
    try {
      await apiService.deletePackage(packageId);
      Swal.fire({
        icon: 'success',
        title: 'Success',
        text: `Package ${roleName} deleted successfully!`,
      });
      const updatedPackages = await apiService.getPackage();
      setPackages(updatedPackages);
      setShowDropdown(null);
      ...
      <div className="overflow-auto rounded-xl mb-5 shadow-[0px_1.97px_6.47px_0px_#00000005,0px_9px_18.2px_0px_#00000008,0px_22.78px_48.83px_0px_#0000000A,0px_45px_112px_0px_#0000000F]">
        <table className="min-w-[1000px] w-full text-left">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan="3" className="text-center text-white py-4">Loading...</td>
              </tr>
            ) : packages.length === 0 ? (
              <tr>
                <td colSpan="3" className="text-center text-white py-4">No packages found</td>
              </tr>
            ) : (
              packages.map((role, index) => (
                <tr
                  key={role.id}
                  className={`bg-[#131060] text-white h-[90px] ${index !== packages.length - 1 ? 'border-b border-[#fff]' : ''}`}
                >
                  <td className="px-5">{role.title}</td>
                  <td className="px-5">{role.price}</td>
                  <td className="px-5 relative">
                    <img
                      src="/tableAction.svg"
                      alt="Action"
                    ...
                    />
                    {showDropdown === index && (
                      <div className="absolute right-72 top-18 bg-[#0B0741] text-white border rounded-tl-[12px] rounded-tr-[4px] rounded-br-[12px] rounded-bl-[12px] z-50 w-[150px]">
                        <div
                          className="px-4 py-2 border-b cursor-pointer hover:bg-[#1A1850]"
                          onClick={() => openEditPopup(role)}
                        >
      ...
      {editPopup.open && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          ...
          >
            ...
            <h2 className="text-[22px] font-semibold">{roleName}</h2>
            ...
            <div className="flex justify-end gap-4 mt-4">
              <button
                ...
                onClick={() => handleDelete(packageId, roleName)}
              >
                Delete Plan
              </button>
              <button
