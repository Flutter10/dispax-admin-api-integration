...
const CustomBranding = () => {
  ...
  useEffect(() => {
    const fetchBrandingData = async () => {
      try {
        setLoading(true);
        const data = await apiService.getAllCustomBranding();
        setBrandingData(Array.isArray(data) ? data : []); // Ensure array
        setLoading(false);
      } catch (err) {
  ...
  const handleCreateRole = async () => {
    if (!companyName || !brandName || !subDomain || !primaryColor || !secondaryColor || !typography) {
      Swal.fire({
        icon: 'error',
        title: 'Validation Error',
        text: 'All fields except Company Logo are required. Please fill in all fields.',
      });
      return;
    }
    ...
    try {
      ...
      await apiService.addCustomBranding(brandingData);
      Swal.fire({
        icon: 'success',
        title: 'Success',
        text: 'Custom branding created successfully!',
      });
      // Refresh branding data
      const updatedData = await apiService.getAllCustomBranding();
      setBrandingData(Array.isArray(updatedData) ? updatedData : []);
      closeCreateRolePopup();
    } catch (error) {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error.message || 'Failed to create custom branding. Please try again.',
      });
    }
  };
    ...
    <>
      ...
      <div className="overflow-auto rounded-xl mb-5 shadow-[0px_1.97px_6.47px_0px_#00000005,0px_9px_18.2px_0px_#00000008,0px_22.78px_48.83px_0px_#0000000A,0px_45px_112px_0px_#0000000F]">
        {loading ? (
          ...
          <p className="text-white text-center py-4">No branding data available</p>
        ) : (
          <table className="min-w-[1000px] w-full text-left">
            <thead>
              <tr className="bg-[#070539] h-[55px] text-white text-sm border-b border-[#fff]">
            ...
            <tbody>
              {brandingData.map((role, index) => (
                <tr
                  key={role._id || index}
                  className={`bg-[#070539] text-white h-[90px] ${index !== brandingData.length - 1 ? 'border-b border-[#fff]' : ''}`}
                >
                  <td className="px-5">{role.companyName}</td>
                  <td className="px-5">{role.brandName}</td>
                  <td className="px-5">
                    <img
                      src={role.logo || '/emoji.svg'}
                      alt="Logo Preview"
                      className="max-w-full max-h-[100px] object-contain"
                    />
                  </td>
                  <td className="px-5">
                    <span className="bg-[#F8F8F840] px-2 py-2 rounded-[10px]">
                      {role.primaryColor}
                    </span>
                  </td>
                  <td className="px-5">
                    <span
                      className={`text-[#F5F5F5] text-[14px] inline-flex items-center justify-center w-[72px] h-[32px] gap-[10px] rounded-[24px] pt-[6px] pr-[12px] pb-[6px] pl-[12px] ${
                        role.status === 'Active'
                          ? 'bg-[#14AE5C]'
                          : role.status === 'Pending'
                          ? 'bg-[#E8B931]'
                  ...
                  <td className="px-5">
                    <span className="bg-[#F8F8F840] px-2 py-2 rounded-[10px]">{role.subDomain}</span>
                  </td>
                  <td className="px-5">{formatDate(role.updatedAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {createRolePopup && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          ...
          >
            ...
            <h2 className="text-[22px] font-semibold mb-4">Create Custom Branding</h2>
            <div className="flex-1 overflow-y-auto pr-2" style={{ marginBottom: '60px' }}>
              <div className="mb-4">
                <label className="text-[12px] text-white mb-1 block">Company Name</label>
                <input
                  type="text"
              ...
              <div className="mb-4">
                <label className="text-[12px] text-white mb-1 block">Subdomain</label>
                <input
                  type="text"
                  value={subDomain}
                  onChange={(e) => setSubDomain(e.target.value)}
                  className="w-full h-[48px] px-4 py-2 bg-transparent text-white border border-[#FFFFFF] rounded-lg focus:outline-none"
                  placeholder="Enter Subdomain (e.g., Bluedart.yourapp.com)"
                  required
                />
              </div>
              <div className="mb-4 flex-1 h-[182px]">
                <label className="text-[12px] text-gray-400 mb-1 block">Company Logo</label>
                <div className="border border-dashed border-[#FFFFFF] rounded-lg p-6 flex flex-col items-center justify-center h-full">
                  {imagePreview ? (
                    <img
                      src={imagePreview}
                      alt="Uploaded Logo"
                      className="max-w-full max-h-[100px] object-contain mb-4"
                    />
                  ) : (
                    <>
                      <svg
                      ...
                      <span className="text-gray-400 mb-4">Upload Logo Here</span>
                    </>
                  )}
                  <label className="px-4 py-2 bg-[#6B46C1] text-white rounded-lg cursor-pointer">
                    Select Photo
              ...
              <div className="mb-4">
                <label className="text-[12px] text-white mb-1 block">Primary Color</label>
                <div
                  className="flex items-center gap-2 h-[48px] px-4 py-2 bg-transparent text-white border border-[#FFFFFF] rounded-lg cursor-pointer"
                  onClick={handlePrimaryColorClick}
                >
                  <div
                    className="w-6 h-6 rounded-full border border-[#FFFFFF]"
