...
import apiService from "../../components/ApiController/ApiService"; // Adjusted import path
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        setDashboardData({
          activeTruck: response.data.activeTruck,
          activeDriver: response.data.activeDriver,
        });
      } catch (err) {
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto custom-scrollbar w-full rounded-xl mb-5 shadow-[0px_1.97px_6.47px_0px_#00000005,0px_9px_18.2px_0px_#00000008,0px_22.78px_48.83px_0px_#0000000A,0px_45px_112px_0px_#0000000F]">
          <table className="min-w-full table-auto text-left">
            <thead>
              <tr className="bg-[#131060] h-[40px] text-white text-sm border-b border-[#fff]">
                <th className="px-5">Active Trucks</th>
                <th className="px-5">Active Drivers</th>
              </tr>
            </thead>
            <tbody>
              <tr className="bg-[#131060] text-white h-[90px]">
                <td className="px-5">
                  <span className="text-[#F5F5F5] text-[14px] inline-flex items-center justify-center w-[100px] h-[40px] gap-[10px] rounded-[24px] pt-[6px] pr-[12px] pb-[6px] pl-[12px] bg-[#14AE5C]">
                    {dashboardData.activeTruck}
                  </span>
                </td>
                <td className="px-5">
                  <span className="text-[#F5F5F5] text-[14px] inline-flex items-center justify-center w-[100px] h-[40px] gap-[10px] rounded-[24px] pt-[6px] pr-[12px] pb-[6px] pl-[12px] bg-[#14AE5C]">
                    {dashboardData.activeDriver}
                  </span>
                </td>
              </tr>
            </tbody>
